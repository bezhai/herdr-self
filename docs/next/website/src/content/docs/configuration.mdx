...
On macOS, `system` uses an embedded native `Herdr.app` (`dev.herdr.notifications`) installed privately under `~/Library/Application Support/Herdr Notifications/`. It requests Notification Center permission and runs without a Dock icon. No Homebrew helper or runtime compiler is required. Agent notification titles include the agent event and machine name. Clicking a targeted notification asks the originating Herdr client to select that machine and focus its pane, tab, or workspace; an exited client, removed target, or restarted server is reported as unavailable. Existing supported host terminals are brought forward on a best-effort basis. If native installation or launch fails, an installed `terminal-notifier` is a fallback; AppleScript notifications are not used. OS Focus and permission settings still control visible delivery. Linux and Windows keep their existing system senders.
