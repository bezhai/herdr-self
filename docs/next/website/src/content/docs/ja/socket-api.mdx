...
| 通知 | `notification.show`, `notification.show_targeted` |
...
`notification.show` は任意の JSON `target` を受け付けます。対象のサポートが必須の場合は、別途広告される `notification.show_targeted` を使用し、`pane_id`、`tab_id`、`workspace_id` のいずれかを含む必須の `target` を指定します。最も具体的な対象が優先されます。任意の `machine_endpoint_id` は受信クライアントが実際の接続元に再設定します。古いサーバーは新メソッドを拒否するため、対象を無視して成功を返すことはありません。対応クライアントがない場合は `unsupported_notification_target` を返します。任意の `notification.targeted.v1` codec が対象とサーバーの boot ID を運び、generation-1 のコア codec は変更しません。

```json
{"id":"req_targeted","method":"notification.show_targeted","params":{"title":"codex needs attention","target":{"pane_id":"ws_1:p1"}}}
```

成功応答はクライアントへのイベント投入を示し、OS のバナー表示を保証しません。macOS のクリックは元のクライアントにのみ戻り、代替 TUI の起動や別クライアントへの転送は行いません。
