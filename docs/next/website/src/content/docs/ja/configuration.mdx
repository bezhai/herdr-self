...
macOS の `system` は内蔵のネイティブ `Herdr.app`（`dev.herdr.notifications`）を `~/Library/Application Support/Herdr Notifications/` にユーザー専用でインストールします。通知センターの許可を要求し、Dock アイコンは表示しません。Homebrew の補助プログラムや実行時コンパイラーは不要です。Agent 通知のタイトルにはイベントとマシン名が含まれます。対象付き通知をクリックすると、通知元の Herdr クライアントがマシンを切り替え、pane、tab、workspace をフォーカスします。クライアント終了、対象削除、サーバー再起動後は利用不可として扱います。対応する起動済みターミナルを可能な範囲で前面に出します。ネイティブのインストールや起動に失敗した場合のみ、インストール済みの `terminal-notifier` にフォールバックします。AppleScript 通知は使用しません。集中モードと通知許可がバナー表示を制御します。Linux と Windows の通知実装は従来通りです。
