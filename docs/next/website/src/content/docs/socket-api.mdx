...
| Notification | `notification.show`, `notification.show_targeted` |
...
`notification.show` accepts an optional JSON `target`. Callers requiring target support should use the separately advertised `notification.show_targeted` method, with a required `target` containing `pane_id`, `tab_id`, or `workspace_id` (most specific wins). `machine_endpoint_id` is optional and is always rebound by the receiving client to its authenticated source connection. A legacy server rejects the new method instead of silently ignoring a required target. A server with no negotiated client returns `unsupported_notification_target`. The optional `notification.targeted.v1` codec carries the target and server boot identity; generation-1 core codecs remain unchanged.

```json
{"id":"req_targeted","method":"notification.show_targeted","params":{"title":"codex needs attention","target":{"pane_id":"ws_1:p1"}}}
```

A successful response means the event was queued to a client, not that the OS displayed a banner. macOS clicks return only to the originating client; they do not start a replacement TUI or focus another client.
