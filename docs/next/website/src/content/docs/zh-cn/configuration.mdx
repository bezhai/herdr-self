...
在 macOS 上，`system` 使用内嵌的原生 `Herdr.app`（`dev.herdr.notifications`），私有安装于 `~/Library/Application Support/Herdr Notifications/`。它会请求通知中心权限，不显示 Dock 图标，无需 Homebrew 辅助程序或运行时编译器。Agent 通知标题包含事件和机器名。点击定向通知后，原客户端切换到对应机器并聚焦 pane、tab 或 workspace；客户端已退出、目标已删除或服务器已重启时会报告不可用。对已运行且受支持的宿主终端，会尽力将其置前。原生安装或启动失败时可回退到已安装的 `terminal-notifier`；不再使用 AppleScript 通知。系统专注模式和权限仍决定是否显示横幅。Linux 和 Windows 保留现有系统通知实现。
