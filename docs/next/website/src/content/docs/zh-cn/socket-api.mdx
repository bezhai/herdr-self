...
`notification.show` 接受可选 JSON `target`。必须支持目标的调用方应使用单独广告的 `notification.show_targeted`，并提供包含 `pane_id`、`tab_id` 或 `workspace_id` 的必填 `target`，按最具体目标优先处理。`machine_endpoint_id` 可选，客户端始终将其绑定到接收事件的实际连接。旧服务器会拒绝新方法，避免忽略必需目标后报告成功。没有已协商客户端时，服务器返回 `unsupported_notification_target`。可选 `notification.targeted.v1` codec 携带目标与服务器 boot 标识；generation-1 核心编码不变。

```json
{"id":"req_targeted","method":"notification.show_targeted","params":{"title":"codex needs attention","target":{"pane_id":"ws_1:p1"}}}
```

成功响应表示事件已排入客户端队列，不代表操作系统已显示横幅。macOS 点击仅返回原客户端，不启动替代 TUI，也不跳转其他客户端。
